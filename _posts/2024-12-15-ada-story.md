...
<body>
  ...
  <script>
    ...
    // 确保页面加载时只显示第一个图表
    document.addEventListener('DOMContentLoaded', () => {
          const group1Charts = document.querySelectorAll('.group1-container');
          group1Charts.forEach(chart => {
            chart.classList.remove('active');
          });
          if (group1Charts[0]) {
            group1Charts[0].classList.add('active');
          }
        
          // 为按钮组 2 激活第一个图表
          const group2Charts = document.querySelectorAll('.group2-container');
          group2Charts.forEach(chart => {
            chart.classList.remove('active');
          });
          if (group2Charts[0]) {
            group2Charts[0].classList.add('active');
          }
    });
