---
layout: post
title: plot example
---


<h1>Diversity Plot Selector</h1>

<!-- Dropdown 1: Strategy Bins -->
<label for="strategy_bins">Choose strategy bins:</label>
<select id="strategy_bins" onchange="updateImage()">
    <option value="[0, 0.4, 0.7, 1]">[0, 0.4, 0.7, 1]</option>
    <option value="[0, 0.25, 0.5, 1]">[0, 0.25, 0.5, 1]</option>
    <option value="[0, 0.33, 0.66, 1]">[0, 0.33, 0.66, 1]</option>
</select>

<!-- Dropdown 2: Successful Percentage -->
<label for="successful">Choose successful percentage:</label>
<select id="successful" onchange="updateImage()">
    <option value="75">Top 25%</option>
    <option value="85">Top 15%</option>
    <option value="95">Top 5%</option>
</select>

<!-- Display Image -->
<div id="image_container" style="margin-top: 20px;">
    <img id="selected_image" src="" alt="Diversity Plot" style="max-width: 100%; height: auto;">
</div>

<!-- JavaScript Logic -->
<script>
    const images = [
        {'path': 'diversity_plot_[0, 0.4, 0.7, 1]_75.png', 'strategy_bins': '[0, 0.4, 0.7, 1]', 'successful': '75'},
        {'path': 'diversity_plot_[0, 0.4, 0.7, 1]_85.png', 'strategy_bins': '[0, 0.4, 0.7, 1]', 'successful': '85'},
        {'path': 'diversity_plot_[0, 0.4, 0.7, 1]_95.png', 'strategy_bins': '[0, 0.4, 0.7, 1]', 'successful': '95'},

        {'path': 'diversity_plot_[0, 0.25, 0.5, 1]_75.png', 'strategy_bins': '[0, 0.25, 0.5, 1]', 'successful': '75'},
        {'path': 'diversity_plot_[0, 0.25, 0.5, 1]_85.png', 'strategy_bins': '[0, 0.25, 0.5, 1]', 'successful': '85'},
        {'path': 'diversity_plot_[0, 0.25, 0.5, 1]_95.png', 'strategy_bins': '[0, 0.25, 0.5, 1]', 'successful': '95'},

        {'path': 'diversity_plot_[0, 0.33, 0.66, 1]_75.png', 'strategy_bins': '[0, 0.33, 0.66, 1]', 'successful': '75'},
        {'path': 'diversity_plot_[0, 0.33, 0.66, 1]_85.png', 'strategy_bins': '[0, 0.33, 0.66, 1]', 'successful': '85'},
        {'path': 'diversity_plot_[0, 0.33, 0.66, 1]_95.png', 'strategy_bins': '[0, 0.33, 0.66, 1]', 'successful': '95'}
    ];

    function updateImage() {
        const strategy_bins = document.getElementById("strategy_bins").value;
        const successful = document.getElementById("successful").value;

        // Find matching image
        const image = images.find(img => 
            img.strategy_bins === strategy_bins && img.successful === successful
        );

        // Update image source
        const imgElement = document.getElementById("selected_image");
        if (image) {
            imgElement.src = image.path;
            imgElement.alt = `Diversity Plot - Strategy Bins: ${strategy_bins}, Successful: ${successful}`;
        } else {
            imgElement.src = "";
            imgElement.alt = "No matching image found.";
        }
    }

    // Set default image on load
    updateImage();
</script>
